...
void main() {
  ...
  // Radial mask to prevent hard edge clipping
  float radialMask = 1.0 - smoothstep(0.35, 0.5, length(p));
  float alpha = smoothstep(0.0, 0.05, max(c.r, max(c.g, c.b))) * uOpacity * radialMask;
...
export default function MagicRings({
  ...
  useEffect(() => {
    ...
    if (!renderer.capabilities.isWebGL2) {
      // Some simple systems might not have WebGL2, but Three.js will usually handle it.
      // We'll proceed as if it might work.
    }
